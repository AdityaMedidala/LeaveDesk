# LeaveDesk

Employees apply for leave. Employers approve or reject. Built with Vue 3, FastAPI, and MongoDB Atlas.

**Live demo:** `https://leave-desk.vercel.app/login` · **API docs:** `https://leavedesk-production.up.railway.app/docs`

---

## Architecture

```mermaid
sequenceDiagram
    participant Browser
    participant Vue as Vue (Vercel)
    participant API as FastAPI (Railway)
    participant DB as MongoDB Atlas

    Browser->>Vue: Load app
    Vue->>API: POST /auth/login
    API->>DB: Verify credentials
    DB-->>API: User document
    API-->>Vue: JWT + role
    Vue->>Vue: Store token, redirect by role
    Vue->>API: Subsequent requests (Bearer token)
    API-->>Vue: Protected data
```

---

## Stack

| Layer    | Tech                        |
|----------|-----------------------------|
| Frontend | Vue 3 + Vite + Tailwind CSS |
| State    | Pinia                       |
| Backend  | FastAPI + python-jose + bcrypt |
| Database | MongoDB Atlas               |
| Deploy   | Railway (API) + Vercel (UI) |

---

## Local Setup

```bash
cp backend/.env.example backend/.env   # fill in MONGO_URI and JWT_SECRET
docker compose up --build
```

| Service  | URL                        |
|----------|----------------------------|
| Frontend | http://localhost:5173       |
| Backend  | http://localhost:8000       |
| API docs | http://localhost:8000/docs  |

**Without Docker:**
```bash
# Backend
cd backend && pip install -r requirements.txt
uvicorn main:app --reload

# Frontend
cd frontend && npm install && npm run dev
```

---

## Environment Variables

| Variable        | Where           | Description                     |
|-----------------|-----------------|---------------------------------|
| `MONGO_URI`     | `backend/.env`  | MongoDB Atlas connection string |
| `JWT_SECRET`    | `backend/.env`  | Secret for signing JWTs         |
| `VITE_API_URL`  | `frontend/.env` | Backend base URL                |

---

## API

### Auth
| Method | Endpoint         | Body                            |
|--------|------------------|---------------------------------|
| POST   | `/auth/register` | `{ email, password, role }`     |
| POST   | `/auth/login`    | form-data: `username, password` |

### Leaves
| Method | Endpoint                 | Role     |
|--------|--------------------------|----------|
| POST   | `/leaves`                | employee |
| GET    | `/leaves/mine`           | any      |
| GET    | `/leaves?status=Pending` | employer |
| PATCH  | `/leaves/{id}/approve`   | employer |
| PATCH  | `/leaves/{id}/reject`    | employer |
| GET    | `/health`                | —        |

---

## Deployment

**Backend → Railway:** New project from GitHub → root directory `backend` → add env vars → start command:
```
uvicorn main:app --host 0.0.0.0 --port $PORT
```

**Frontend → Vercel:** Import repo → root directory `frontend` → add `VITE_API_URL` → deploy.